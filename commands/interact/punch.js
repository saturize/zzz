...
exports.run = async (client, message, args) => {
    const query = 'anime punch someone';
    ...
    if (!userToPunch) {
        return message.reply('Veuillez mentionner quelqu\'un que vous voulez frapper.');
    }
        ...
        if (response.data && response.data.results && response.data.results.length > 0) {
            ...
            if (imageUrl) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`${message.author} a donné un coup de poing à ${userToPunch}!`)
                    .setImage(imageUrl);
...
exports.name = "punch";
exports.description = "Frappe un utilisateur.";